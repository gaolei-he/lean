...
theorem pred_Ico_choose_eq_pred_pow_from_1_to_1(n : ℕ)(h : 2 ≤ n)(ico_choose_eq_two_pow : ∑ s in Ico 0 (n - 1), Nat.choose (n - 2) s = 2 ^ (n - 2)) :
   (n - 1) * ∑ s in Ico 0 (n - 1), Nat.choose (n - 2) s + 2 ^ (n - 1) = (n - 1) * 2 ^ (n - 2) + 2 ^ (n - 1) := by
  rw[ico_choose_eq_two_pow]
