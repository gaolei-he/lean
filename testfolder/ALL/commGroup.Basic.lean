...
theorem div_eq_of_eq_mul'0 {a b c : G} (h : a = b * c) : a / b = c := by
  rw [h, div_eq_mul_inv, mul_comm, inv_mul_cancel_left]

theorem mul_div_mul_left_eq_div0 (a b c : G) : c * a / (c * b) = a / b := by
  rw [div_eq_mul_inv, mul_inv_rev, mul_comm b⁻¹ c⁻¹, mul_comm c a, mul_assoc, ←mul_assoc c,
    mul_right_inv, one_mul, div_eq_mul_inv]


theorem mul_eq_of_eq_div'0 (h : b = c / a) : a * b = c := by
  rw [h, div_eq_mul_inv, mul_comm c, mul_inv_cancel_left]

theorem div_div_self'0 (a b : G) : a / (a / b) = b := by simpa using mul_inv_cancel_left a b

theorem div_eq_div_mul_div0 (a b c : G) : a / b = c / b * (a / c) := by simp [mul_left_comm c]

theorem div_div_cancel0 (a b : G) : a / (a / b) = b :=
  div_div_self' a b

theorem div_div_cancel_left0 (a b : G) : a / b / a = b⁻¹ := by simp

theorem eq_div_iff_mul_eq''0 : a = b / c ↔ c * a = b := by rw [eq_div_iff_mul_eq', mul_comm]


theorem div_eq_iff_eq_mul'0 : a / b = c ↔ a = b * c := by rw [div_eq_iff_eq_mul, mul_comm]



theorem mul_div_cancel'''0 (a b : G) : a * b / a = b := by rw [div_eq_inv_mul, inv_mul_cancel_left]



theorem mul_div_cancel'_right0 (a b : G) : a * (b / a) = b := by
  rw [← mul_div_assoc, mul_div_cancel''']



theorem div_mul_cancel''0 (a b : G) : a / (a * b) = b⁻¹ := by rw [← inv_div, mul_div_cancel''']

theorem mul_mul_inv_cancel'_right0 (a b : G) : a * (b * a⁻¹) = b := by
  rw [← div_eq_mul_inv, mul_div_cancel'_right a b]


theorem mul_mul_div_cancel0 (a b c : G) : a * c * (b / c) = a * b := by
  rw [mul_assoc, mul_div_cancel'_right]


theorem div_mul_mul_cancel0 (a b c : G) : a / c * (b * c) = a * b := by
  rw [mul_left_comm, div_mul_cancel', mul_comm]

theorem div_mul_div_cancel''0 (a b c : G) : a / b * (c / a) = c / b := by
  rw [mul_comm]; apply div_mul_div_cancel'


theorem mul_div_div_cancel0 (a b c : G) : a * b / (a / c) = b * c := by
  rw [← div_mul, mul_div_cancel''']


theorem div_div_div_cancel_left0 (a b c : G) : c / a / (c / b) = b / a := by
  rw [← inv_div b c, div_inv_eq_mul, mul_comm, div_mul_div_cancel']



theorem div_eq_div_iff_mul_eq_mul0 : a / b = c / d ↔ a * d = c * b := by
  rw [div_eq_iff_eq_mul, div_mul_eq_mul_div, eq_comm, div_eq_iff_eq_mul']
  simp only [mul_comm, eq_comm]



theorem div_eq_div_iff_div_eq_div0 : a / b = c / d ↔ a / c = b / d := by
