...
theorem div_mul_div_cancel'6(a b c : G)(goal:a / b * (b / c) = a / c):a * b⁻¹ * (b * c⁻¹) = a / c := by
rw [div_eq_mul_inv, div_eq_mul_inv] at goal
rw[goal]


theorem div_mul_div_cancel'9(a b c : G)(goal:a / b * (b / c) = a / c):a * (b⁻¹ * (b * c⁻¹)) = a * c⁻¹ := by
simp only [div_eq_mul_inv, mul_assoc] at goal
rw[goal]


theorem div_mul_div_cancel'11(a b c : G)(goal:a / b * (b / c) = a / c):a * b⁻¹ * (b * c⁻¹) = a * c⁻¹ := by
simp only [div_eq_mul_inv] at goal
rw[goal]


theorem div_left_inj3:b * a⁻¹ = c * a⁻¹ ↔ b = c := by
rw [←div_eq_mul_inv, ←div_eq_mul_inv]
rw[div_left_inj]


theorem div_left_inj14:c * a⁻¹ = b * a⁻¹ ↔ b = c := by
rw [←eq_comm, ←div_eq_mul_inv, ←div_eq_mul_inv]
rw[div_left_inj]


theorem div_left_inj15:b * a⁻¹ = c / a ↔ b = c := by
rw [←div_eq_mul_inv]
rw[div_left_inj]


theorem div_left_inj49:c = b * a⁻¹ * a ↔ b = c := by
rw [←mul_inv_eq_iff_eq_mul]
rw [←eq_comm, ←div_eq_mul_inv, ←div_eq_mul_inv]
rw[div_left_inj]


theorem div_left_inj77(goal:b / a = c / a ↔ b = c):c = b * (a⁻¹ * a) ↔ b = c := by
rw [div_eq_mul_inv, div_eq_mul_inv, eq_comm] at goal
rw [mul_inv_eq_iff_eq_mul] at goal
simp only [mul_assoc, mul_inv_cancel_right] at goal
rw[goal]


theorem div_left_inj82(goal:b / a = c / a ↔ b = c):c = b ↔ b = c := by
rw [div_eq_mul_inv, div_eq_mul_inv, eq_comm] at goal
rw [mul_inv_eq_iff_eq_mul] at goal
simp [mul_assoc] at goal
rw[goal]




theorem div_mul_cancel'''12(a b : G):a * (b * a)⁻¹ = b⁻¹ := by
rw [← div_eq_mul_inv]
rw [div_mul_cancel''']


theorem mul_inv_cancel_right2(a b : G)(goal:a * b * b⁻¹ = a):a * 1 = a := by
rw [mul_assoc, mul_right_inv] at goal
rw[goal]


theorem div_div_div_cancel_right'7(a b c : G)(goal:a / c / (b / c) = a / b):a * (c⁻¹ * (b * c⁻¹)⁻¹) = a * b⁻¹ := by
simp only [div_eq_mul_inv, mul_assoc] at goal
rw[goal]
