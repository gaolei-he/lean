...
def split_theorems(file_path):
    # 读取文件内容
    with open(file_path, 'r', encoding='utf-8') as file:
        lean_code = file.read()

    # 使用正则表达式找到所有的定理
    theorems = re.findall(r'(theorem [^\s]*.*?:= by.*?)(?=theorem|$)', lean_code, re.DOTALL)

    # 确保输出目录存在
    output_dir = '/home2/wanglei/Project/testfolder/root/adalean_rl'
    ...
    for theorem in theorems:
        ...
        if match:
            ...
            theorem_code = re.sub(r':= by.*', proof_replacement, theorem, flags=re.DOTALL)
...
def split_theorems0(file_path):
    ...
    # 确保输出目录存在
    output_dir = '/home2/wanglei/Project/testfolder/root/adalean'
    os.makedirs(output_dir, exist_ok=True)
    ...
    for theorem in theorems:
        ...
        if match:
            ...
            # 替换证明步骤为 ':= by lean_repl sorry'
            proof_replacement = ':= by lean_repl sorry'
            theorem_code = re.sub(r':= by.*', proof_replacement, theorem_code, flags=re.DOTALL)
...
lean_dir = "/home2/wanglei/Project/AdaLean"
file_list = list_files(lean_dir)

for i, file in enumerate(file_list):
    # 指定要读取的Lean代码文件路径
    input_file_path = "/home2/wanglei/Project/AdaLean/" + file

    # 分离定理并生成文件
    split_theorems0(input_file_path)
