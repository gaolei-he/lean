...
theorem prod_Ico_succ_to {a b : ℕ} (hab : a ≤ b) (f : ℕ → β) :
    (∏ k in Ico a (b + 1), f k) = (∏ k in Ico a b, f k) * f b := by lean_repl sorry
    rw [mul_comm]

example {a b : ℕ} (hab : a ≤ b) (f : ℕ → β)(goal: (∏ k in Ico a (b + 1), f k) = (∏ k in Ico a b, f k) * f b) : ∏ k in Ico a (b + 1), f k = (∏ k in Ico a b, f k) * f b := by
    rw [mul_comm] at goal
    rw [mul_comm] at goal
    rw [goal]
